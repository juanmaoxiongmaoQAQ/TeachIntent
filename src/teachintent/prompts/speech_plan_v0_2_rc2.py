"""Speech Plan Generator Prompt — **v0.2-rc.2** (minimal correction of v0.2-rc.1).

This is a narrow, prompt-level correction. It does NOT change:
...
What v0.2-rc.2 corrects, relative to v0.2-rc.1
----------------------------------------------
Development QC found a delivery-mode collapse in v0.2-rc.1: over the frozen
30-case set it output ``delivery_plan = {}`` for 30/30 cases, i.e. the model
learned "sparse control" as "always output `{}`". This module corrects the
over-correction: sparsity means *minimum justified control*, NOT *zero control*.

The Delivery Necessity Gate no longer says "Assume you need NO delivery control";
it now starts from the minimum necessary delivery plan and forbids choosing
``{}`` merely to avoid over-specification. A small positive-guidance section and
a balanced delivery self-check (verify controls AND verify emptiness) are added.

Everything else is preserved byte-for-byte from v0.2-rc.1: the six-intent
minimum adequacy, verbal-first principle, content boundary, hard/adversarial
intent discipline, anti-injection, sparse-control prohibitions, the internal
self-check discipline (no reasoning/chain-of-thought in the output), and the
field contract.

Versioning: ``PROMPT_VERSION = "v0.2-rc.2"``. v0.1 (``speech_plan.py``) and
v0.2-rc.1 (``speech_plan_v0_2_rc1.py``) are untouched and remain callable.
Selection among all three is explicit through ``teachintent.prompts.registry``.

Like v0.1 and v0.2-rc.1, this module is a pure, deterministic function of the
validated input document. It never introspects the Pydantic models.
"""
...
# ---------------------------------------------------------------------------
# v0.2-rc.2 behavioral additions (P1 / P2 / P3 + internal self-check).
# ---------------------------------------------------------------------------
_PLANNING_ORDER = """\
...
3. Start from the minimum necessary delivery plan: `delivery_plan = {}` is preferred when the wording alone fully carries the pedagogical function, but do NOT choose `{}` merely to avoid over-specification.
...
Core rule:
Start from the minimum necessary delivery plan. Add a delivery control only when a specific pedagogical need clearly requires vocal realization beyond what the verbal wording alone can achieve.

An empty `delivery_plan` is preferred when the wording alone fully carries the pedagogical function, but do NOT choose `{}` merely to avoid over-specification. If vocal realization materially contributes to the intended learner-state change, include the smallest justified control set.

Sparsity does NOT mean always empty. The goal is minimum justified control, not zero control.
...
_POSITIVE_GUIDANCE = """\
# When minimum delivery control may be justified

A SMALL control set MAY be justified in situations such as the following. These are illustrations, NOT templates: they do NOT mean "add delivery whenever affective_state is present". Each still requires a specific pedagogical reason.

- repeated misconception with learner frustration: one reassuring-but-corrective attitudinal stance;
- an uncertain learner during scaffolding: one gentle, non-pressuring stance, if needed to preserve learner agency;
- misconception contrast: one local prominence target when auditory contrast materially aids discrimination;
- supportive feedback after frustration: one restrained supportive stance if wording alone would sound flat or dismissive.

For every such control, still answer: "What specific pedagogical function does this control serve?" If there is no clear answer, omit it."""
...
D6. Do not stack redundant controls. One sufficient control beats several redundant ones. If a single control already carries the pedagogical function (e.g. `attitudinal_tone = "reassuring but corrective"`), do not additionally add `emotion = "calm"`, `speaking_rate = "slow"`, prominence, or boundary for the same function."""
...
4. Delivery (present) — For every delivery control, is there a specific pedagogical reason that wording alone cannot achieve?
5. Delivery (absent) — If `delivery_plan` is empty, have I omitted a vocal realization that materially contributes to the intended learner-state change?

If the answer to check 4 is NO, remove that control. If the answer to check 5 is YES, add the smallest justified control. Do NOT emit any of these checks, any chain-of-thought, or any commentary. Output ONLY the final JSON object."""
...
_INTRO = (
    ...
    "This is Prompt v0.2-rc.2. Relative to v0.1 it makes 'sparse but justified delivery "
    "control' explicit and enforceable; it strengthens verbal instructional adequacy and "
    "intent discipline. It corrects v0.2-rc.1's over-correction toward always-empty "
    "delivery_plan: sparsity means minimum justified control, not zero control. It preserves "
    "v0.1's strong intent fidelity, content faithfulness, learner compatibility, and "
    "delivery-pedagogy alignment. It does NOT change the output schema."
)
...
def build_speech_plan_prompt(input_doc: dict) -> SpeechPlanPrompt:
    """Build the v0.2-rc.2 Speech Plan Generator prompt for a validated input doc.
    ...
    delimiters) is shared with v0.1 and v0.2-rc.1; only the system instruction
    differs.
    """
