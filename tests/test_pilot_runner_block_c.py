"""Focused mocked tests for the Block C (hard_adversarial) pilot baseline runner.

No real API calls. The frozen Block C dataset is loaded but only
...
# ---------------------------------------------------------------------------
# Block C runs 6/6 successfully.
# ---------------------------------------------------------------------------


def test_block_c_runs_6_6_successfully(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    manifest = run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)

    assert manifest.case_count == 6
    assert manifest.pass_count == 6
    assert manifest.fail_count == 0
    assert manifest.block == "hard_adversarial"
    assert all(c["outcome"] == "success" for c in manifest.cases)
    run_dir = tmp_path / manifest.run_id
    assert (run_dir / "manifest.json").exists()
    assert len(list((run_dir / "cases").iterdir())) == 6


def test_block_c_metadata_includes_block(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    manifest = run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)

    case_dir = tmp_path / manifest.run_id / "cases" / manifest.cases[0]["case_id"]
    metadata = json.loads((case_dir / "metadata.json").read_text(encoding="utf-8"))
    assert metadata["block"] == "hard_adversarial"
    assert metadata["case_id"] == "PILOT-C-ELI-01"
...
# ---------------------------------------------------------------------------
# Block C runs in dataset file order.
# ---------------------------------------------------------------------------


def test_block_c_runs_in_dataset_file_order(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    manifest = run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)
    ...
    assert actual_ids[0] == "PILOT-C-ELI-01"
    assert actual_ids[-1] == "PILOT-C-EXT-01"
...
def test_block_c_only_runtime_input_passed_to_generation(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)

    assert fake.call_count == 6
    cases = _load_block_c_cases()
    for index, user in enumerate(fake.users):
        ...
        assert "hard_adversarial" not in user
        assert "PILOT-C-" not in user
        assert cases[index]["case_id"] not in user


def test_block_c_prompt_is_v0_1(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)

    case_dir = tmp_path / run_dir_name(tmp_path) / "cases" / "PILOT-C-ELI-01"
...
def test_block_c_structural_preflight_failure_zero_model_calls(
    tmp_path: Path,
) -> None:
    """A structurally invalid Block C dataset aborts before any model call."""
    cases = _load_block_c_cases()
    # Break the wrapper: add contrast_group to tags (Block A field, not allowed
    # in Block C).
    cases[0]["tags"]["contrast_group"] = "anchor_01"
    broken_path = tmp_path / "broken_block_c.jsonl"
...
def test_block_c_wrong_model_zero_model_calls(tmp_path: Path) -> None:
...
def test_block_c_wrong_base_url_zero_model_calls(tmp_path: Path) -> None:
...
def test_block_c_structured_output_enabled_zero_model_calls(
    tmp_path: Path,
) -> None:
    fake = FakePilotClient(
...
def test_block_c_one_api_failure_preserved_and_continues(
    tmp_path: Path,
) -> None:
    def content_fn(idx):
        if idx == 0:
...
# ---------------------------------------------------------------------------
# One parsing/schema/Pydantic failure preserved; remaining cases continue.
# ---------------------------------------------------------------------------


def test_block_c_one_validation_failure_preserved_and_continues(
    tmp_path: Path,
) -> None:
    invalid_plan = {
        "schema_version": "1.0.0-rc.3",
    ...
    assert manifest.pass_count == 5
    assert manifest.fail_count == 1
    assert manifest.cases[2]["outcome"] == "SpeechPlanStructuralError"
    assert manifest.cases[3]["outcome"] == "success"
    # raw_response + parsed preserved for the failed case.
    case_dir = tmp_path / manifest.run_id / "cases" / manifest.cases[2]["case_id"]
...
# ---------------------------------------------------------------------------
# Artifacts go under results/pilot/block_c/<run_id>/ (registry).
# ---------------------------------------------------------------------------


def test_block_c_registry_paths() -> None:
    entry = PILOT_BLOCKS["block_c"]
    assert entry["dataset_path"] == BLOCK_C_DATASET_PATH
    assert entry["results_dir"].name == "block_c"
    assert entry["results_dir"].parent.name == "pilot"
    assert entry["results_dir"].parent.parent.name == "results"
    # Block A and Block B registries are intact.
    assert PILOT_BLOCKS["block_a"]["dataset_path"] == BLOCK_A_DATASET_PATH
    assert PILOT_BLOCKS["block_a"]["results_dir"].name == "block_a"
    assert PILOT_BLOCKS["block_b"]["dataset_path"] == BLOCK_B_DATASET_PATH
    assert PILOT_BLOCKS["block_b"]["results_dir"].name == "block_b"


# ---------------------------------------------------------------------------
# Generic CLI registry recognizes block_c.
# ---------------------------------------------------------------------------


def test_generic_cli_recognizes_block_c() -> None:
    assert "block_c" in PILOT_BLOCKS
    config = PILOT_BLOCKS["block_c"]
    assert "dataset_path" in config
    assert "results_dir" in config
    assert config["dataset_path"].name == "block_c_hard_adversarial.jsonl"
    assert config["results_dir"].name == "block_c"


# ---------------------------------------------------------------------------
# Manifest and metadata record block = "hard_adversarial".
# ---------------------------------------------------------------------------


def test_block_c_manifest_records_hard_adversarial(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    manifest = run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)

    manifest_data = json.loads(
        (tmp_path / manifest.run_id / "manifest.json").read_text(encoding="utf-8")
    )
    assert manifest_data["block"] == "hard_adversarial"
    assert manifest_data["case_count"] == 6
    assert manifest_data["pass_count"] == 6
    assert manifest_data["fail_count"] == 0
...
def test_block_c_no_credentials_in_artifacts(tmp_path: Path) -> None:
    fake = FakePilotClient(_all_success_content)
    manifest = run_pilot_block(fake, BLOCK_C_DATASET_PATH, tmp_path)
...
# ---------------------------------------------------------------------------
# Block A and Block B existing behavior remains unchanged.
# ---------------------------------------------------------------------------


def test_block_a_alias_still_works_with_block_c_present(
    tmp_path: Path,
) -> None:
    """run_pilot_block_a (backward-compat alias) still behaves identically."""
    fake = FakePilotClient(_all_success_content)
...
def test_block_b_still_works_with_block_c_present(tmp_path: Path) -> None:
    """Block B runner still behaves identically with Block C in the registry."""
    fake = FakePilotClient(_all_success_content)
