...
import {
  SegmentedBatonCandidate,
  SegmentedResponsePlayer,
} from "./SegmentedBatonCandidate";
...
const result: SegmentedBatonRenderResponse = {
  ...
  segments: [4, 2, 1, 3].map((order) => ({
    segment_id: `seg_0${order}`,
    order,
    status: "success",
    audio_url: `/segment-${order}.wav`,
    duration_seconds: 1,
    text_sha256: "hash",
    text_char_count: 10,
    mapped: {},
...
describe("segmented playback", () => {
  ...
  it("preloads in order, plays from first, advances on ended, and finishes without looping", async () => {
    ...
    expect(audios.map((a) => a.getAttribute("src"))).toEqual(
      [1, 2, 3, 4].map((i) => `/segment-${i}.wav`),
    );
    expect(audios.every((a) => a.preload === "auto")).toBe(true);
    expect(HTMLMediaElement.prototype.play).not.toHaveBeenCalled();
    await user.click(screen.getByText("试听完整语音"));
    expect(vi.mocked(HTMLMediaElement.prototype.play).mock.instances).toEqual([
      audios[0],
    ]);
    for (let i = 0; i < 3; i++) {
      fireEvent.ended(audios[i]);
      expect(
        vi.mocked(HTMLMediaElement.prototype.play).mock.instances.at(-1),
      ).toBe(audios[i + 1]);
    }
    ...
    expect(screen.getByText("试听完整语音")).toBeEnabled();
    fireEvent.ended(audios[3]);
    expect(HTMLMediaElement.prototype.play).toHaveBeenCalledTimes(4);
  });
  ...
  it("stop prevents advancement and restart begins at segment one", async () => {
    ...
    await user.click(screen.getByText("从头播放"));
    expect(
      vi.mocked(HTMLMediaElement.prototype.play).mock.instances.at(-1),
    ).toBe(audios[0]);
    unmount();
  ...
  it.each(["partial_failure", "failed", "interrupted"] as const)(
    "does not play an incomplete %s response",
    (status) => {
      render(<SegmentedResponsePlayer result={{ ...result, status }} />);
      expect(screen.getByText("试听完整语音")).toBeDisabled();
      expect(screen.getByText("从头播放")).toBeDisabled();
      expect(screen.getByRole("status")).toHaveTextContent("部分语音未能生成");
      expect(HTMLMediaElement.prototype.play).not.toHaveBeenCalled();
    },
  );

  it("stops on play rejection and on audio loading failure 不会跳过", async () => {
    const user = userEvent.setup();
    ...
    const { container } = render(<SegmentedResponsePlayer result={result} />);
    await user.click(screen.getByText("试听完整语音"));
    expect(await screen.findByRole("alert")).toHaveTextContent("无法开始播放");
    expect(screen.getByText("停止")).toBeDisabled();
    await user.click(screen.getByText("试听完整语音"));
    fireEvent.error(container.querySelectorAll("audio")[1]);
    expect(screen.getByRole("alert")).toHaveTextContent("已停止播放");
    expect(screen.getByText("停止")).toBeDisabled();
    expect(HTMLMediaElement.prototype.play).toHaveBeenCalledTimes(2);
  });
  ...
  it("render calls only the candidate API and playback never resynthesizes", async () => {
    ...
    await user.click(screen.getByText("生成语音"));
    await screen.findByText("语音已生成，点击试听。");
    await user.click(screen.getByText("试听完整语音"));
    await user.click(screen.getByText("从头播放"));
  ...
  it("discards an old session response after unmount", async () => {
    ...
    const { rerender } = render(
      <SegmentedBatonCandidate key="old" sessionId="old" />,
    );
    await user.click(screen.getByText("生成语音"));
    ...
    await waitFor(() =>
      expect(screen.queryByText("试听完整语音")).not.toBeInTheDocument(),
    );
    expect(screen.getByText("生成语音")).toBeEnabled();
  });
  ...
  it("stops the old queue when a new render starts", async () => {
    ...
    const { container } = render(
      <SegmentedBatonCandidate sessionId="session-1" />,
    );
    await user.click(screen.getByText("生成语音"));
    await screen.findByText("语音已生成，点击试听。");
    await user.click(screen.getByText("试听完整语音"));
